...
//    함수의 본문으로 "기계어 치환"을 수행합니다.
// 2) 인라인 최적화는 컴파일 최적화 명령입니다.
//    사용자가 inline의 키워드를 지정하여도, 무조건 인라인 치환이 수행되지 않습니다.
//     MSVC: /Ob1
// 3) 인라인 함수를 사용하면, 일반 함수에 비해 코드 메모리 사용량이 증가할 수 있습니다.
// 4) 함수의 길이가 긴 경우, 인라인 치환이 수행되지 않을 수 있습니다.
// => 인라인 함수는 간결하게 만드는 것이 좋습니다.

// 핵심: 인라인 함수와 매크로 함수의 의도(함수 호출 오버헤드 제거)가 동일하다면,
//      인라인 함수가 선호되어야 합니다.
#if 1
#define SQUARE(x) ((x) * (x))
...
// 인라인 함수는 반드시 헤더 파일을 통해서 제공되어야 합니다.
